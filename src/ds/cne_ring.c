 ...
 */
static int
cne_ring_setup(cne_ring_t *r, unsigned count, unsigned flags)
{
    struct cne_ring *_ring = r;
...
cne_ring_t *
cne_ring_init(void *addr, ssize_t size, unsigned int esize, unsigned int count,
              unsigned int flags)
{
    void *ring_mem;
    struct cne_ring *r;
    ...
    cne_ring_setup(r, requested_count, flags);
    r->ring_mem = ring_mem;
...
cne_ring_t *
cne_ring_create(unsigned int esize, unsigned int count, unsigned int flags)
{
    return cne_ring_init(NULL, 0, esize, count, flags);
}
