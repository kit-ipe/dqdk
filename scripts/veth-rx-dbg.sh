...
while sleep 1; do
    values_now=$(ethtool -S $NIC | grep -w "rx_queue_0_xdp_packets\|rx_queue_0_xdp_redirect\|rx_queue_0_xdp_drops" | awk '{print $2}' ORS=' ')
    rx_xsk_packets_now=$(echo $values_now | awk '{print $1}')
    rx_xsk_xdp_redirect_now=$(echo $values_now | awk '{print $2}')
    rx_xdp_drop_now=$(echo $values_now | awk '{print $3}')
    ...
    if [[ "$rx_rdr" -gt "0" ]]; then
        ...
        columns="PPS,Redirects,XDP Drop"
        echo -e "$rx_pps\t\t$rx_rdr\t\t$rx_xdp_drp\t\t" | column --table --table-columns "$columns" --output-separator "|"
    fi
